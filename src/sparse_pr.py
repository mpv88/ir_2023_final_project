...
class SparsePageRank:
    def __init__(self):
        '''Initialize the SparsePageRank class'''
    ...
    def load_network(self, dataset_path: str, is_undirected: bool = False) -> None:
        '''Load the graph dataset from the given directory'''
        edges = self._load_edges(f"{dataset_path}/edges.tsv")
    ...
    def _load_edges(self, edge_file_path: str) -> np.ndarray:
        '''Load edges from the given file path'''
    ...
    def load_node_names(self, dataset_path: str) -> None:
        '''Load node labels from the given directory'''
        label_file_path = f"{dataset_path}/node_labels.tsv"
        self.node_labels = pd.read_csv(label_file_path, sep="\t")
        
    def get_adj_row_sums(self) -> np.ndarray:
        '''Get the row sums of the normalized adjacency matrix'''
